...
class PokemonGameInfoMenu < PokemonPauseMenu
	def pbStartPokemonMenu
		...
		infoCommands[cmdQuests = infoCommands.length] = _INTL("Quest Log")
		...
		loop do
			...
			if cmdTrainer >= 0 && infoCommand == cmdTrainer
				...
				pbMessageDisplay(msgwindow, _INTL("The level cap can be raised by reading Battle Reports (a consumable item)."))
				pbMessageDisplay(msgwindow, _INTL("You'll earn Battle Reports after defeating Gym Leaders and major story battles."))
          ...
          pbFadeOutIn do
			...
			elsif cmdQuests > - 1 && infoCommand == cmdQuests
				pbPlayDecisionSE
				pbFadeOutIn {

...
完胜机制，对战结束时，若队伍中有在战斗中濒死的精灵，则视为失败
...
42.现在加入了礼物系统
43.现在可以输入hyena1获得1只路边的野狗
44.现在可以输入psyduck10获得1只10级的我最喜欢的死鸭子
45.现在可以输入pokeball5获得5个精灵球
46.现在在精灵导航中无需进入对战即可直接获得精灵
47.现在可以按F快速骑上自行车
48.现在和训练师的战斗结束后所有精灵会回复30%HP
49.现在和训练师的战斗结束后所有精灵会回复异常
50.现在和训练师的战斗结束后所有精灵会回复全部技能的全部PP
51.修改Documents菜单选项名为Tools
52.现在同一种类的精灵，玩家只能拥有一只，获得已拥有的精灵时，该精灵将会被传送到Dimension D中，当条件允许时可取回该精灵
53.加入了时间胶囊功能，玩家可以将精灵放入时间胶囊中，时间胶囊中的精灵可以在所有存档之间存取，但是从时间胶囊中取回的精灵的等级将不会超过玩家当前阶段的等级上限
54.加入了战斗模拟器功能，可随时将玩家当前的队伍加载到战斗模拟器中，然后镜像对战或者使用其他队伍进行对战
55.现在战斗模拟器可加载的队伍数量没有上限，不仅可以随时删除，并且全存档通用
56.现在战斗模拟器也可以加载其他玩家配置的队伍了
57.现在战斗模拟器支持自定义对战模式了（1v1/2v2/1v2/2v2）
58.现在战斗模拟器支持记录对战过的训练师的队伍了，可以自行选择是否记录
59.现在随机模式已被禁用
60.修改了部分按键说明文本以和当前游戏匹配
61.现在在选择主角外观时将不允许取消选择
62.修改了科学胖子的对话以和当前游戏匹配
63.修复了初始城镇精灵中心里护士对话文本的一个语法错误
64.取消了在使用电脑的取出功能时点击队伍出现的无意义的文本
65.修复了某些特性的不正确的描述文本
66.增加了特性图鉴，现在可以在特性图鉴中查看游戏中所有的特性了
LOGGER

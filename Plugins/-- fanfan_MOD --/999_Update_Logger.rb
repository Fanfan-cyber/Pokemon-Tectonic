...
特性记录器功能，在和训练师的对战中，特性记录器会在对手出场时记录对手的特性，玩家可以用特性记录器中记录的特性来修改自己的精灵的特性
...
4.还原在之前的更新中对地图事件的更改
5.取消了对精灵特性的合法性检测，因为这会使某个NPC的特性变为非预期的特性，并且为了之后的更新内容，这也是必须取消的
6.现在当精灵的物种或者形态改变时，特性会正确的改变和发动了
7.将Displaying Ability（主显特性）改为Main Ability，即主特性，默认情况下主特性为随机一个物种特性，物种特性即为该精灵的物种所拥有的特性
8.现在主特性为精灵的常驻特性，主特性在一定条件下可以修改，也只能通过手动修改来改变主特性
9.现在精灵的特性分为主特性、物种特性和额外特性，精灵拥有的所有特性都会同时生效，但是相同的特性只会生效一个
...
14.增加了特性记录器，在和训练师的对战中，特性记录器会在对手出场时记录对手的特性
15.现在特性记录器不会记录和战斗模拟器中对战的对手的特性
16.现在可以在队伍菜单中的Modify（修改）中打开特性记录器，使用特性记录器中记录下的特性来修改精灵的主特性
17.

      ...
      pbActivateWindow(@sprites,"itemlist") {
        loop do
          ...
          if itemwindow.sorting
            ...
            elsif Input.trigger?(Input::SPECIAL) || MInput.trigger?(Settings::ITEM_SEARCH_KEY) # Input.triggerex?(Settings::ITEM_SEARCH_KEY) # Search form
              searchText = pbEnterText(_INTL("Type name / description!"), 0, 999)
              thispocket = @bag.pockets[itemwindow.pocket]
              if searchText != ""

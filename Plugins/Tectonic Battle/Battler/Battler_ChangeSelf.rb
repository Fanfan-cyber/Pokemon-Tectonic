...
class PokeBattle_Battler
    ...
    def apply_faint_healing
        ...
        #healing_turn = TA.get(:doublebattle) ? Settings::FAINT_HEALING_TURN_DOULBE : (TA.get(:customrevivalturn) || Settings::FAINT_HEALING_TURN)
        healing_turn = @battle.singleBattle? ? (TA.get(:customrevivalturn) || Settings::FAINT_HEALING_TURN) : Settings::FAINT_HEALING_TURN_DOULBE
        @battle.pbDisplay(_INTL("{1} will revive in {2} turns!", pbThis, healing_turn))
    ...
    def pbChangeTypes(newType)
        ...
        disableEffect(:Roost)
        refreshDataBox
        @battle.clear_adaptive_ai_data(self) # recalc if type changed
    end

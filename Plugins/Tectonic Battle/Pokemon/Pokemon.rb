...
class Pokemon
    ...
    def recalculateAbilityFromIndex
        ...
        if abil_index >= 2 # Hidden ability
            ...
            @ability = sp_data.legalAbilities[abil_index] || sp_data.legalAbilities[0]
        end
    end
...
=end
    ...
    def legalItems?(itemSet, showMessages = false)
        ...
        if itemSet.length != itemSet.uniq.length
            pbMessage(_INTL("{1} can't hold two of the same item!", name, getItemName(:CRYSTALVEIL))) if showMessages
            return false
        end

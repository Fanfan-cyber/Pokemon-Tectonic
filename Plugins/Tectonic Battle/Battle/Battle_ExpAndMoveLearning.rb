...
class PokeBattle_Battle
    ...
    #=============================================================================
    def pbGainExp
        return if @autoTesting || TA.get(:zeroexp) || TA.get(:battle_loader)
    ...
    def pbGainExpOne(idxParty, defeatedBattler, numPartic, expShare, expAll, hasExpJAR, showMessages = true)
        ...
        exp *= Settings::EXP_RATE_BOOST # TectoQuake bonus
    ...
    def pbLearnMove(idxParty, newMove)
        ...
        loop do
            pbDisplayPaused(_INTL("{1} wants to learn {2}{4}, but it already knows {3} moves.",
                pkmnName, moveName, pkmn.moves.length.to_word, pkmn.move_matched?(newMove) ? _INTL("(matched)") : ""), no_highlighting: true)
